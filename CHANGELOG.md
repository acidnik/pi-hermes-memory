...
- **The review delta pointer survives process restarts and session resumes**: `lastReviewedEntryId` is now persisted per session in a new `review_progress` table (SQLite) and restored at the start of the next run, so a quit+resume (which reuses the session id) continues from where the previous process stopped instead of re-reviewing the whole branch. The full branch is delta-reviewed exactly once per session lifetime — the first run without a pointer. The pointer is written through only on a successful review (failed transports leave it untouched and retry the same fragment), only in delta mode (`reviewDeltaOnly:false` never reads or writes it), and is deliberately not cleared on `session_compact` or quit.

### Added

- **Opt-in auto-retrieval of memories before user messages** ([#231](https://github.com/chandra447/pi-hermes-memory/issues/231)): enable with `autoRetrieve: { "enabled": true }`. Each user message triggers a cheap FTS5 `memory_search`, and the top-K matches are delivered as a **custom message** the model sees as a user-role text block — the user's own message text is never modified, so the injection is not glued into the bubble they typed. The transcript renders that message through a registered renderer: collapsed it shows only the entry count plus a few keywords, and the standard output-expand key (`app.tools.expand`, default `ctrl+o`) expands it to the full block. Scope is the **active project plus global** (`project IS NULL`) memories only — other projects' facts are never injected. Every row is injected **at most once per session**: the dedup is persisted in a new `retrieved_memories` table keyed by session id, so it survives process restarts and session resumes; `session_compact` resets it (post-compaction the model has forgotten the facts, so they become injectable once more); session end (quit) does NOT clear it, so an exit+resume does not re-flood the same facts. **Freshly written memories are marked as already injected for the current session** — a fact saved explicitly (`memory_add`) or extracted by review/flush/correction is not shown again by auto-retrieval in the same session. Config: `topK` (default 3), `maxChars` (1500), `targets` (all), `minQueryChars` (12). Off by default — no behavior change until enabled.

- **Opt-in memory retrieval on bash tool calls**: enable with `bashRetrieve: { "enabled": true }`. When the model runs a bash command, the command is reduced to search terms and FTS5-searched against memory; the top-K matches are delivered as a **separate custom message** (via pi's steer queue) that is injected right after the bash tool output and before the model's next continuation — the bash tool result itself is left untouched, and the transcript renders the block through the shared auto-retrieve renderer as a collapsible/clickable `🧠 Retrieved N entries` widget. Term extraction follows the proposed convention: tokens starting with `-` (flags) or `/` (absolute paths / switch-style keys) are dropped, along with shell syntax, env assignments, shell builtins and modifiers (`sudo`, `cd`, `echo`, `2>&1`…); what remains is command names, relative paths and filenames (relative-path basenames and file-extension stems are derived, e.g. `src/store/db.ts` → `db.ts`). Scope is the active project plus global memories. **Dedup is shared with auto-retrieve**: every injected row goes into the same `retrieved_memories` set (at most once per session across both features), so repeated commands like `npm run check` do not re-inject the same facts; `session_compact` resets the set, after which facts become injectable again. Session end (quit) keeps the set (a resumed session reuses the same session id, so it does not re-flood). Config: `topK` (default 4), `maxChars` (1500), `targets` (all), `minTerms` (1). Off by default — no behavior change until enabled.
...
- **SQLite is now the primary memory write target in policy-only mode** (default): `memory_add` / `memory_replace` / `memory_remove` persist each target scope straight into the SQLite `memories` table before anything else. The Markdown files are demoted to an optional human-readable mirror: the new `markdownMirror` setting (default `true`) keeps writing `MEMORY.md` / `USER.md` / `failures.md` as before; set it to `false` to stop touching those files entirely. With the mirror disabled, `MemoryStore` reads the authoritative scope back from SQLite, so entries written by another session with the mirror off stay visible. The character cap and auto-consolidation gate now live on the legacy Markdown path only — policy-only writes are unbounded by design, and the mirror is a plain atomic overwrite (no conflict/retry loop), which removes the Markdown write-gate failure modes behind the rapid-write retry issue (#214). `legacy-inject` mode is unchanged: the files remain the injected source of truth and are always mirrored.
